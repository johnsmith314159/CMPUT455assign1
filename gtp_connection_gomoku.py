        ...
        def gogui_rules_final_result_cmd(self, args):
            ...
            for i in range(len(final_list)):
                if final_list[i]==True and col_check != 1:
                    checkpoint=i
                    for i in range(checkpoint,len(final_list),+(self.board.size+1)):
                        if final_list[i]==True:
                            match_col_black+=1
            ...
            for i in range(len(final_list_dia)):
                if final_list_dia[i]==True and dia_f_check!=1:
                    checkpoint=i
                    for i in range(checkpoint,len(final_list_dia),+(self.board.size+2)):
                        if final_list_dia[i] == True:
            ...
            for i in range(len(final_list_dia)):
                if final_list_dia[i]==True and dia_b_check!=1:
                    checkpoint=i
                    for i in range(checkpoint,len(final_list),+(self.board.size)):
                        if final_list_dia[i]==True:
    ...
    def play_cmd(self, args):
        ...
        try:
            ...
            if color != self.board.current_player:
                self.error(
                    "Wrong player turn.".format(move, args[1])
                )
                return
            ...
            if not self.board.play_move(move, color):
                self.respond("Illegal Move: {}. Coordinate occupied.".format(board_move))
                return
            else:
                self.debug_msg(
            ...
            #Test if coordinate is within boardsize.
            coord = move_to_coord(args[1], self.board.size)
            if coord:
                move = coord_to_point(coord[0], coord[1], self.board.size)
            else:
                self.error(
                    "Error executing move {} converted from {}. Wrong coordinate.".format(move, args[1])
                )
                return
        except Exception as e:
